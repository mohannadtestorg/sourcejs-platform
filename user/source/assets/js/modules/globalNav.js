...
define([
    ...
    var isProductAccessible = function (productPath) {
        if (window.location.pathname.indexOf('/specs/products/') !== -1) {
            var remain = window.location.pathname.replace('/specs/products/', '');
            if (remain === '#' || remain === '') {
                if (auth && auth.prods) {
                    var pname = getProductFromPath(productPath);
                    if (auth.prods.indexOf(pname) !== -1)
                        var accessible = true;
                    return auth.isAtyponEmployee || accessible;
                }
                return false;
            }
            return true;
    ...
    GlobalNav.prototype.renderNavigationList = function (catalog, data) {
        ...
        var filter = function (spec) {
            ...
            var canAccess = typeof auth === 'undefined' || isProductAccessible(spec.url);
            return canAccess && !(isInIgnoreList || isFiltered);
        };
    ...
    GlobalNav.prototype.renderNavTreeItem = function (itemData) {
        ...
        if (itemData.clearview) {
            itemDataUrl += '?clarify=true&tpl=clear';
        }
    ...
    GlobalNav.prototype.renderPreviewsToggler = function () {
        ...
        if (initPreviewValue === "true") { // initPreviewValue is string, not boolean
            ...
            $filter.append(this.templates.togglePreviewLink({
                "classes": classes,
                "togglePreviewLabel": labels.hidePreview
            }));
        } else {
            $filter.append(this.templates.togglePreviewLink({
                "classes": classes,
                "togglePreviewLabel": labels.showPreview
            }));
        }
    ...
    GlobalNav.prototype.renderSortFilters = function () {
        ...
        var enabledFilter = JSON.parse(localStorage.getItem("source_enabledFilter")) || {
