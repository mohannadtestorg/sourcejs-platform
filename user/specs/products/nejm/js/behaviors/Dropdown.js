A17.Behaviors.Dropdown = function(container) {

  var active = false;
  var allow = true;
  var hoverable = (container.getAttribute('data-dropdown-hoverable') !== null);
  var hovered = false;
  ...
  function _open(event) {
    ...
    A17.Functions.setFocusOnTarget(container.querySelector('ul'));
    min$(document).trigger('dropdown:closed', { el: container });
    min$(document).trigger('tooltip:hide');
    min$(document).trigger('tooltip:disallow', { el: container });
    active = true;
  }
  ...
  function _close(event) {
    ...
    min$(document).trigger('dropdown:closed', { el: container });
    min$(document).trigger('tooltip:allow', { el: container });
    active = false;
  }
  ...
  function _focus(event) {
    if (document.activeElement === container || container.contains(document.activeElement)) {
      ...
      if (!active) {
        ...
        _close(event);
        A17.Functions.setFocusOnTarget(container.parentNode);
      }
    } else {
      _close(event);
    }
